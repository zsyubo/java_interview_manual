...
## ## `JVM调优`
**从代码上减少GC的开销**
1. 不要显示的作死调用`System.gc()`。
2. 临时对象能复用就复用，虽然新生代GC不会发生STW，但是GC也是消耗资源的。
3. 对象不用时，显示设置为null，可以让GC收集器不用去判断，直接定性为垃圾。
4. 在做字符串拼接时，劲量使用StringBuilder。String底层拼接用的是StringBuilder.append，但是默认是每次append会new一次，产生过多的临时对像。
5. 减少使用包装类型，毕竟包装类型也是对象，也会参与GC的。所以少用。
6. 减少静态对象变量，静态对象会一直存在在内存中。
7. 减少大对象的创建，避免新生代没有足够的空间来存在，直接晋升为老年代。

**`JVM参数调优`** 
1. -Xms、-Xmx一般设置相同值，防止垃圾收集器杂最大、最小之间收缩堆而产生额外的时间。jvm底层是C++，一开始设置的话，直接使用jvm划分好的的内存，不去在想系统申请。
2. 调整堆到适合的大小，建议在线程应用运行一段时间后查看堆中对象大小。jstat可以查看到老年代大小。Xmx 和 Xms设置为老年代存活对象的3-4倍，即FullGC之后的老年代内存占用的3-4倍。毕竟需要留一些亢余空间。Sun官方建议年轻代的大小为整个堆的3/8左右， 所以按照上述设置的方式，基本符合Sun的建议。

> https://www.jianshu.com/p/d23e7197d3fa JVM内存设置多大合适？Xmx和Xmn如何设置？
> https://juejin.im/post/5d200b54f265da1bac40384a jvm讲解+调优
